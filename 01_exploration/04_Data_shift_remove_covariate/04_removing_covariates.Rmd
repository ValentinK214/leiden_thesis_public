...
The goal of this experiment was to investigate the impact of **missing covariates** in the German dataset on **rank instability**, using the Dutch dataset as the fixed reference.  

The reference Dutch dataset has **no missing covariates** (`missing_covs_global_NL = "0"`). The German dataset varies in the number of **missing covariates** (`missing_covs_global_DE = "2"`). The `var_imp_ranking_global` determines the order in which covariates are removed, which impacts the order (`terms_matching_global`) in which terms are removed from the true signal. 
...
- **Inner loop** (`n_missing_cov_global = 3`): Within each repetition, vary the number of missing covariates in the German dataset (0, 1, or 2), while keeping all other parameters fixed.  
...
missing_covs_global_NL = "0" 
missing_covs_global_DE = "2" # 2 is max, otherwise glmnet cannot be fitted

n_missing_cov_global = c("0", "1", "2")

B_global = 50 


var_imp_ranking_global = list( "x4", "x1", "x2", "x3")

terms_matching_global = list("x4" = "+ exp(0.3 * x4) * (1 + 0.9 * sin((2 * pi/3) * x4))",
                     "x3" = "+ x3",
                     "x2" = "x2 +",
                     "x1" = "x1 +")
```
...
```{r}
true_signal = function(x1 = 0, x2 = 0, x3 = 0, x4 = 0) {
  x1 + x2 + x3 + exp(0.3 * x4) * (1 + 0.9 * sin((2 * pi / 3) * x4))
}

true_function_string_global = paste(deparse(body(true_signal)[[2]]), collapse = " ")

# Dynamically update the terms order based on variable importance order
terms_matching_global = terms_matching_global[unlist(var_imp_ranking_global)]
```


## New true signal

The goal of this code is to create a modified version of the "true signal" for the German dataset 
by removing a specified number of covariates.

The function `create_new_fn_body()` takes three arguments:  
- `n_missing_covs`: the number of covariates to remove,  
- `terms_matching`: the functional forms associated with each covariate, and  
- `input_body`: the original true signal as a string.  

It removes the top `n_missing_covs` covariates (based on importance ranking) 
from `input_body` by literal substring replacement.  

The result `new_true_signal_body_DE` is a string representing the German signal 
after removing the specified covariates.

The function `new_fn_creater()` converts this modified signal string into an actual R function, 
which can be evaluated for inputs `x1`, `x2`, `x3`, and `x4`.  
This function is then used as the "Orcale" algorithm later on.


```{r}
create_new_fn_body = function(n_missing_covs, terms_matching, input_body) {
  if (n_missing_covs %in% c("1", "2", "3")) {
    
    remove_top_n_terns = list( "1" = c(terms_matching[[1]] ),
                               "2" = c(terms_matching[[1]], terms_matching[[2]] ),
                               "3" = c(terms_matching[[1]], terms_matching[[2]], terms_matching[[3]] ) )
  
    term_to_be_removed = remove_top_n_terns[[n_missing_covs]]
    term_to_be_removed
  
    for (t in term_to_be_removed) {
      # fixed=TRUE makes it literal substring removal
      input_body = gsub(t, "", input_body, fixed = TRUE)
    }
...
for (n_missing in 0:3) {
  print(create_new_fn_body(n_missing, terms_matching_global, true_function_string_global) )
}

```

Create new true signal

```{r}
new_true_signal_body_DE = create_new_fn_body(missing_covs_global_DE, terms_matching_global, true_function_string_global)


new_fn_creater = function(new_signal_body) {
  eval(parse(text = sprintf(
  "function(x1=0, x2=0, x3=0, x4=0) { %s }", new_signal_body)))
}

new_fn_creater(new_true_signal_body_DE)
```
...
4. **n_missing_cov** (integer): number of top covariates to remove based on variable importance (0–2).  

**Returns:**  
- **X**: numeric matrix (n × (4 − n_missing_cov)) of predictors, with top covariates removed if specified  
...
2. Identify the top `n_missing_cov` covariates to remove based on `var_imp_ranking_global` and remove them from `X`.  
...
4. Compute error variance for the requested noise proportion, take square root to obtain **irreducible error** -> raw irreducible error based on `irreducible_error` input  
5. Form `y = true_signal(...) + Normal(0, sd)` using all original covariates (even those removed from `X`).
6. Return a list containing the predictor matrix `X`, response `y`, `signal_var`, and `irreducible_error`.
...
generate_data = function(noise = noise_global, 
                         ...
                         n_missing_cov) {
  if (n_missing_cov > 3) stop("n_missing_cov cannot be greater than 3")
  ...
  # Identify the covariate that needs to be removed based on variable importance
  remove_top_n_imp_vars = list("0" = c(NA),
                             "1" = c(var_imp_ranking_global[[1]] ),
                             "2" = c(var_imp_ranking_global[[1]], var_imp_ranking_global[[2]] ),
                             "3" = c(var_imp_ranking_global[[1]], var_imp_ranking_global[[2]], var_imp_ranking_global[[3]] ) )

  cov_to_be_removed = remove_top_n_imp_vars[[n_missing_cov]]
  
  # Create original dataframe
  X = data.frame(x1 = x1, x2 = x2, x3 = x3, x4 = x4)
  
  # Remove variables if required
  X = X[ , !(names(X) %in% cov_to_be_removed), drop = F] 
...
df_Dutch = generate_data(use_original_signal_var = F, noise = noise_global_NL, 
                         n_missing_cov = missing_covs_global_NL) # noise_global
df_German = generate_data(use_original_signal_var = F, noise = noise_global_DE,
                          n_missing_cov = missing_covs_global_DE) # 
...
# New code:
for (n_missing in n_missing_cov_global) {
  # Generate data
  df_tmp = generate_data(use_original_signal_var = F, 
                         noise = noise_global_NL, n_missing_cov = n_missing)
  plot_dataset(df_tmp, paste("N missing", n_missing), "blue")
}
...
LM NOT useful anymore as "observed" NOISE proportion changes.
...
```{r}
(df_Dutch$irreducible_error)^2 / var(df_Dutch$y)

(df_German$irreducible_error)^2 / var(df_German$y)
```
...
```{r}
# Quick sanity check of the input format
X = df_German$X
head(X)
...
4. **n_missing_cov** (integer / character): number of top covariates removed from the signal/data (0–2).  

**Returns:**  
- A data frame (7 × 10) summarizing model performance across 9 metrics (MSE, NSE, RSE, RRMSE, NRMSE, MAPE, PR, MinMax, NDRMSE) for 7 algorithms:  
  - True signal (oracle) — **using the modified local true-signal that reflects `n_missing_cov`**  
  - Dummy/null model  
  - Elastic net  
  - Random forest  
  - Linear regression  
...
2. **Create a local true-signal function that reflects missing covariates**  
   - Define `true_signal_all()` which applies `local_fn_new_true_signal` row-wise to a matrix `X` to produce oracle (noiseless) predictions that match the current missing-covariate configuration.  
...
```{r}
generate_summary_table = function(X, y, training_percentage = 0.75, n_missing_cov) {
  # Make sure that a row vector is treated as a matrix
  X = as.matrix(X)
  ...
  ## True signal
  # Create new true signal function body & convert to local function
  new_true_signal_body_local = create_new_fn_body(n_missing_cov, terms_matching_global, true_function_string_global)
  local_fn_new_true_signal = new_fn_creater(new_true_signal_body_local)
  
  true_signal_all = function(X) {
    y = numeric(nrow(X))
    for (idx in 1:nrow(X)) {
      tmp_row = as.matrix(X[idx, ] )
      y[idx] = do.call(local_fn_new_true_signal, as.list(tmp_row)) # Match named vector to arguments
    }
    return(y)
  }
  ...
  model_elastic = glmnet(x = as.matrix(X[train, ]),
                         y = y[train],
  ...
  ## SVM
  model_svm = svm(
    x = as.matrix(X[train, ]),    # training predictors
  ...
  ## True signal
  # Make prediction
  true_signal_pred = true_signal_all(as.matrix(X[test, ]))
  # Evaluate
  ...
  ## Elatic net
  # Make predictions
  elastic_pred = stats::predict(model_elastic, newx = as.matrix(X[test, ]))
  # Evaluate predictions
  ...
  ## SVM
  # Make predictions
  svm_pred = predict(model_svm, newdata = as.matrix(X[test, ]))
  # Evaluate predictions
...
# Sanity check
df_Dutch_summary = generate_summary_table(X = df_Dutch$X, y = df_Dutch$y, n_missing_cov = missing_covs_global_NL)
df_Dutch_summary

df_German_summary = generate_summary_table(X = df_German$X, y = df_German$y, n_missing_cov = missing_covs_global_DE)
df_German_summary
```
...
**Purpose:**  
Repeat the experiment **`B_global` times** to account for sampling variance, while systematically varying only the number of missing covariates for the German dataset (non-reference).  

- **Outer loop**: Represents one full replication (i.e. one Monte Carlo run) of the experiment -> Dutch dataset (reference) is held fixed.  
- **Inner loop**: Iterates over different numbers of missing covariates for the German dataset while keeping all other parameters fixed.  
...
pb = progress_bar$new(format = "(:spin) [:bar] :percent [Elapsed time: :elapsedfull || Estimated time remaining: :eta]",
                       total = B_global * length(n_missing_cov_global),
...
for (idx in 1:B_global) {
  ...
  df_Dutch = generate_data(use_original_signal_var = FALSE, noise = noise_global_NL, n_missing_cov = missing_covs_global_NL)
  df_Dutch_summary = generate_summary_table(X = df_Dutch$X, y = df_Dutch$y, n_missing_cov = missing_covs_global_NL)
  
  
  for (n_missing_cov in n_missing_cov_global) {
    # Generate datasets
    df_German = generate_data(use_original_signal_var = FALSE, noise = noise_global_DE, n_missing_cov = n_missing_cov)

    # Generate summary tables
    df_German_summary = generate_summary_table(X = df_German$X, y = df_German$y, n_missing_cov = n_missing_cov)
    ...
    df_DE_longer$n_missing_cov = n_missing_cov
    ...
    RD_df$n_missing_cov = n_missing_cov
    ...
    rank_cor_df$n_missing_cov = n_missing_cov
...
filename_ranks = paste0(
  "ranks_",
  timestamp,
  "_ref_n-missing=", missing_covs_global_NL,
  "_B=", B_global,
  ".Rdata"
)
...
ggplot(data = sim_1, mapping = aes(x = n_missing_cov, y = score, color = model, group = model)) +
  geom_line(na.rm = T) +
...
ggplot(data = sim_1_no_dummy, mapping = aes(x = n_missing_cov, y = score, color = model, group = model)) +
  geom_line(na.rm = T) +
  facet_wrap(~ metric, scales = "free_y") +
  labs(title = "Sim 1")

# ONLY RF, Linear, Dummy
sim_1_rf_lin_true = sim_model_results_df %>% 
  filter(simulation == 1, model %in% c("Linear_reg", "XGB", "Dummy_R2"))

ggplot(data = sim_1_rf_lin_true, mapping = aes(x = n_missing_cov, y = score, color = model, group = model)) +
  geom_line(na.rm = T) +
  facet_wrap(~ metric, scales = "free_y")
```
...
ggplot(sim_1_MSE, aes(x = n_missing_cov, y = score, color = model, group = model)) +
  geom_line()
```
...
summary_df = sim_model_results_df %>%
  ...
  group_by(model, metric, n_missing_cov) %>%
  summarise(
...
ggplot(summary_df, aes(x = n_missing_cov, y = mean_score, color = model, fill = model, group = model)) +
  geom_line() +
...
Warning is due to dummy model for PR not having any values for mean and sd. Ribbon sees the NA values and throws warning.
...
# Summarize: mean and sd of score over simulations
summary_df = sim_model_results_df %>%
  filter(model %in% c("Linear_reg", "XGB", "Dummy_R2")) %>%
  group_by(model, metric, n_missing_cov) %>%
  summarise(
    mean_score = mean(score, na.rm = TRUE),
    sd_score = sd(score, na.rm = TRUE),
...
# Fix legend
selected_model_labels = c("Dummy_R2" = "Null Model",
...
ggplot(summary_df, aes(x = n_missing_cov, y = mean_score, color = model, fill = model, group = model)) +
  ...
  labs(
    ...
    subtitle = paste0("Mean score ± 1 SD over ", B_global, " simulations, with noise proportion ", 
                      noise_global_DE),
    x = "Number of missing covariates in df",
    y = "Score",
    color = "Model",
    fill = "Model"
  ) +
...
algo_plot = ggplot(summary_df %>% filter(metric %in% c("MSE", "NSE", "NRMSE", "RRMSE", "PR")), 
       aes(x = n_missing_cov, y = mean_score, color = model, fill = model, group = model)) +
  geom_line() +
  ...
  facet_wrap(~ metric, scales = "free_y") +
  labs(
    title = "Performance Across Missingness Levels and Algorithms",
    subtitle = paste0("Mean performance ± 1 SD over ", B_global, " Monte Calro simulations"),
    x = "Number of Missing Covariates",
...
# Convert model to factor with desired order
  mutate(model = factor(model, levels = c("Linear_reg", "Dummy_R2", "XGB")))

selected_model_labels = c("Dummy_R2" = "Mean Guesser",
                 "Linear_reg" = "Linear Regression",
                 "XGB" = "XGBoost")
...
ggplot(data = sim_1, mapping = aes(x = n_missing_cov, y = rank_cor, color = metric)) +
  geom_point(na.rm = T) +
  facet_wrap(~ metric, scales = "free_y")
...
# Plot: with ribbon for ± SD
ggplot(summary_df, aes(x = n_missing_cov, y = mean_rank_cor, color = metric, fill = metric)) +
  geom_point() +
  ...
  facet_wrap(~ metric) +
    labs(
    title = "Rank Correlation Across Number of Missing Covariates",
    subtitle = paste("Mean rank correlation ± 1 SD over", B_global, "simulations",
                     "\nReference dataset is the full dataset with 4 covariates"),
    x = "Number of missing covariates (compared to full dataset)",
    y = "Rank Correlation",
    color = "Model",
    caption = "Removed covariates from full dataframe in variable importance order, so the most important variable first.",
    fill = "Model"
  ) +
  theme_minimal() +
...
viz_df = viz_df %>% group_by(n_missing_cov) %>% 
  arrange(n_missing_cov, rank_cor) %>% 
...
viz_df$n_missing_cov_factor = factor(viz_df$n_missing_cov, 
                                           levels = sort(unique(viz_df$n_missing_cov)))
...
p_boxplot = ggplot(viz_df, aes(x = n_missing_cov_factor, y = rank_cor)) +
...
```{r, dpi=300, fig.width=8, fig.height=5}
p_boxplot +
  labs(subtitle = paste0("With n = ", missing_covs_global_NL, " correlated features as reference level"),
       caption = "")
```
...
z_score_table = viz_df %>% group_by(n_missing_cov) %>% 
...
names(z_score_table) = c("corr_num", 'sample prop P(tau >= 0.4)', "sample prop < 0.899" )
z_score_table
```
