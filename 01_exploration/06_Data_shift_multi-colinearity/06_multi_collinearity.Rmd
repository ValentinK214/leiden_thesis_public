---
title: "Adding collinear covariates"
...
The goal of this experiment was to investigate the impact of **adding collinear covariates** in the German dataset on **rank instability**, using the Dutch dataset as the fixed reference.  


The Dutch dataset is generated with a fixed number of correlated features (`corr_num_global_NL = 0`), while the German dataset varies in the number of correlated features (`corr_num_global_DE = 4`), with a fixed correlation structure (`rho_global = 0.9`).  
...
- **Inner loop** (`num_correlated_features_global = 4`): Within each repetition, vary the German dataset’s **number of correlated covariates**, while keeping all other parameters fixed.  
...
num_correlated_features_global = 4
rho_global = 0.9

corr_num_global_NL = 0
corr_num_global_DE = 4
...
```{r}
get_correlated_feature = function(x_input, rho) {
  k = length(x_input)
  z = rnorm(k)  # independent noise
  
  x_scaled = scale(x_input)  # standardize x4_vals
  y_scaled = rho * x_scaled + sqrt(1 - rho^2) * z
  
  # If you want to bring it back to original scale:
  as.numeric(scale(y_scaled) * sd(x_input) + mean(x_input))
}
```
...
4. **num_corr** (integer): number of additional correlated features to generate based on `x4`.  
5. **rho** (numeric): correlation coefficient used when generating correlated features.  

**Returns:**  
- **X**: numeric matrix (n × (4 + num_corr)) of predictors, including any correlated features  
...
2. If `num_corr > 0`, generate additional correlated predictors based on `x4`:  
   - Loop over `nums` from 1 to `num_corr`  
   - Compute a new feature using `get_correlated_feature(x4, rho)`  
   - Append each new feature as a column to the predictor matrix `X`  
3. If `use_original_signal_var = FALSE`:  
...
5. Form `y = true_signal(...) + Normal(0, sd)` 
...
generate_data = function(noise = noise_global, 
  ...
  if (num_corr > 0) {
    for (nums in 1:num_corr) {
      print(nums)
      # Compute correlated feature
      new_x = get_correlated_feature(x4, rho)
      col_name = paste("x", 4 + nums, sep = "")
      new_x_df = data.frame(setNames(list(new_x), col_name))
      X = cbind(X, new_x_df)
    }
  }
...
df_Dutch = generate_data(use_original_signal_var = F, noise = noise_global_NL,
                         num_corr = corr_num_global_NL, rho = 0)
df_German = generate_data(use_original_signal_var = F, noise = noise_global_DE,
                          num_corr = corr_num_global_DE, rho = rho_global)
...
```{r}
# Sanity check
cor(df_German$X[, 4], df_German$X[, 8])
```
...
# New code:
for (num_cor in 1:num_correlated_features_global) {
  # Generate data
  df_tmp = generate_data(use_original_signal_var = F, 
                         noise = noise_global_NL, num_corr = num_cor, rho = rho_global)
  plot_dataset(df_tmp, paste("Num corr", num_cor), "blue")
}
...
**CODE EXPLANATION**: Nested simulation over number of correlated features  

**Purpose:**  
Repeat the experiment **`B_global` times** to account for sampling variance, while systematically varying the number of correlated features for the German dataset (non-reference).  
- **Outer loop**: Represents one full replication (ie. MC run) of the experiment -> Dutch dataset (reference) is held fixed.  
- **Inner loop**: Iterates over the number of correlated features for the German dataset while keeping all other parameters fixed.  
...
pb = progress_bar$new(format = "(:spin) [:bar] :percent [Elapsed time: :elapsedfull || Estimated time remaining: :eta]",
                       total = B_global * (num_correlated_features_global+1),
...
for (idx in 1:B_global) {
  # Number of correlated features must start at 0
  for (nums_cor_idx in 0:num_correlated_features_global) {
    # Generate datasets
    ...
    df_German = generate_data(use_original_signal_var = FALSE, noise = noise_global_DE,
                              num_corr = nums_cor_idx, rho = rho_global)
    ...
    df_DE_longer$corr_num = nums_cor_idx # corr_num
    ...
    rank_cor_df$corr_num = nums_cor_idx
...
filename_ranks = paste0(
  "ranks_",
  timestamp,
  "_ref_n-correlated=", corr_num_global_NL,
  "_rho=", rho_global,
  "_B=", B_global,
  ".Rdata"
)
...
ggplot(data = sim_1, mapping = aes(x = corr_num, y = score, color = model)) +
  geom_line(na.rm = T) +
...
ggplot(data = sim_1_no_dummy, mapping = aes(x = corr_num, y = score, color = model)) +
  geom_line(na.rm = T) +
  facet_wrap(~ metric, scales = "free_y") +
  labs(title = "Sim 1")
...
ggplot(data = sim_1_rf_lin_true, mapping = aes(x = corr_num, y = score, color = model)) +
  geom_line(na.rm = T) +
  facet_wrap(~ metric, scales = "free_y")
```
...
ggplot(sim_1_MSE, aes(x = corr_num, y = score, color = model)) +
  geom_line()
```
...
summary_df = sim_model_results_df %>%
  ...
  group_by(model, metric, corr_num) %>%
  summarise(
...
ggplot(summary_df, aes(x = corr_num, y = mean_score, color = model, fill = model)) +
  geom_line() +
  geom_ribbon(aes(ymin = mean_score - sd_score, ymax = mean_score + sd_score), 
              alpha = 0.2, color = NA) +
  facet_wrap(~ metric, scales = "free_y") +
    labs(
    title = "Model Performance Over Collinear Features",
    subtitle = paste("Mean score ± 1 SD over", B_global, "simulations"),
    x = "Number of Features Correlated to x4",
    y = "Score",
    color = "Model",
    fill = "Model"
  ) +
...
ggplot(summary_df, aes(x = corr_num, y = mean_score, color = model, fill = model, group = model)) +
  geom_line() +
  geom_ribbon(aes(ymin = mean_score - sd_score, ymax = mean_score + sd_score), 
...
algo_plot = ggplot(summary_df %>% filter(metric %in% c("MSE", "NSE", "NRMSE", "RRMSE", "PR")), 
       aes(x = corr_num, y = mean_score, color = model, fill = model, group = model)) +
  geom_line() +
  ...
  facet_wrap(~ metric, scales = "free_y") +
  labs(
    title = "Performance Across Multicollinearity Levels and Algorithms",
    subtitle = paste0("Mean performance ± 1 SD over ", B_global, " Monte Calro simulations"),
    x = paste0("Number of correlated features to x4 (with rho = ", rho_global, ")"),
...
ggplot(data = sim_1, mapping = aes(x = corr_num, y = rank_cor, color = metric)) +
  geom_point(na.rm = T) +
  facet_wrap(~ metric, scales = "free_y")
...
# Plot: with ribbon for ± SD
ggplot(summary_df, aes(x = corr_num, y = mean_rank_cor, color = metric, fill = metric)) +
  geom_point() +
  geom_ribbon(aes(ymin = mean_rank_cor - sd_rank_cor, ymax = mean_rank_cor + sd_rank_cor), 
              alpha = 0.2, color = NA) +
  facet_wrap(~ metric) +
    labs(
    title = "Rank Correlation Over Collinear Features",
    subtitle = paste0("Mean score ± 1 SD over ", B_global, " simulations, with noise proportion ", 
                      noise_global_DE,
                      "\nTrue function: ", tru_function_string,
                      "\nCorrelation Rho: ", rho_global,
                      "\nReference dataset has no collinear features"),
    x = "Number of Features Correlated to x4",
    y = "Rank Correlation",
    color = "Model",
    fill = "Model"
...
viz_df = viz_df %>% group_by(corr_num) %>% 
  arrange(corr_num, rank_cor) %>% 
...
viz_df$corr_num_factor = factor(viz_df$corr_num, 
                                           levels = sort(unique(viz_df$corr_num)))
...
p_boxplot = ggplot(viz_df, aes(x = corr_num_factor, y = rank_cor)) +
  ...
  labs(color = paste0("Rank within\neach multicollinearity level\n(", B_global, " draws)")) +
  theme_minimal() +
  labs(
    title = "Rank Stability Across Multicollinearity Levels",
    subtitle = paste0("Each dot = 1 Kendall's τ from 1 draw; ", B_global, " Monte Carlo draws per n/p level",
                      "\nReference corr num level: ", corr_num_global_NL,
                      "\nTrue function:", paste(deparse(body(true_signal)[[2]] )) ),
    x = paste0("Number of additional correlated features to x4 (with rho = ", rho_global, ")"),
    y = "Kendall's τ",
    caption = paste(
...
```{r, dpi=300, fig.width=8, fig.height=5}
p_boxplot +
  labs(subtitle = paste0("With n = ", corr_num_global_NL, " correlated features as reference level"),
       caption = "")
```
...
z_score_table = viz_df %>% group_by(corr_num) %>% 
